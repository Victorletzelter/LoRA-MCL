...
class CustomTensorboardLogger(TensorBoardLogger):
    ...
    def save_and_close(self) -> None:
        ...
        # Convert temporary all metrics recursively to numpy arrays
        for key in self._all_metrics:
            if isinstance(self._all_metrics[key], dict):
                for subkey in self._all_metrics[key]:
                    self._all_metrics[key][subkey] = _convert_value(self._all_metrics[key][subkey])
            else:
                self._all_metrics[key] = _convert_value(self._all_metrics[key])
