...
from local_transformers.pytorch_utils import Conv1D
...
class LoraLayer(BaseTunerLayer):
    ...
    def loftq_init(self, adapter_name):
        from local_peft.utils.loftq_utils import loftq_init
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        if getattr(lora_config, 'use_moe_lora', False):
            ...
            kwargs.update(lora_config.loftq_config)
            new_module = MoELoraLinear(target, adapter_name=adapter_name, num_experts=num_experts, init_zero_router=init_zero_router, stochastic_router=stochastic_router, expert_specific=expert_specific, fast_version=fast_version, sparse_moe_enabled=sparse_moe_enabled, top_k_sparse_moe=top_k_sparse_moe, **kwargs)
        else:
            if kwargs["fan_in_fan_out"]:
                warnings.warn(
                    "fan_in_fan_out is set to True but the target module is `torch.nn.Linear`. "
...
class MoELoraLinear(nn.Module, LoraLayer):
    ...
    def __init__(self, base_layer, adapter_name: str, num_experts: int, init_zero_router: bool = False, r: int = 0, lora_alpha: int = 1, lora_dropout: float = 0.0,
                 fan_in_fan_out: bool = False, init_lora_weights: bool = True, stochastic_router=None, expert_specific=None, fast_version: bool = False, sparse_moe_enabled: bool = False, 
                 top_k_sparse_moe: int = 1, **kwargs):
        super().__init__()
        LoraLayer.__init__(self, base_layer, **kwargs)
        self.fan_in_fan_out = fan_in_fan_out
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        torch_result_dtype = base_out.dtype # Save original dtype
        orig_shape = x.shape
        if x.dim() == 2:
        ...
        # Set activate adapter always to lora0 (the experts are inside lora0)
        ...
        # Cast input to router's weight dtype
        ...
        if self.fast_version and not self.sparse_moe_enabled: 
            if (self.expert_specific != "true" and self.stochastic_router != "true"):
                ...
                ### No need to compute the router in this case             
                ### Do a forward only for expert{int(active_idx)}
                if self.training:
                ...
                moe_lora = self.lora_B[active_adapter][f"expert{int(active_idx)}"](lora_a) * self.scaling[active_adapter] # [batch, seq, out_features]
                ...
                router_weights_2d = router_weights.view(-1, num_experts)  # [batch*seq, num_experts]
                expert_choice_flat = torch.multinomial(router_weights_2d, 1, replacement=True)  # [batch*seq, 1]
                expert_choice = expert_choice_flat.view(batch_size, seq_len, 1)  # [batch, seq, 1]

                # Forward only the chosen expert per token
                moe_lora = x_.new_zeros(batch_size, seq_len, self.out_features)
                choice = expert_choice.squeeze(-1)  # [batch, seq]
                ...
                for i in range(self.num_experts):
                    ...
                    x_sel = x_[b_idx, s_idx]  # [N_i, in_features]
                    if self.training:
                        x_sel = self.lora_dropout[active_adapter](x_sel)
                    h = self.lora_A[active_adapter][f"expert{i}"](x_sel)     # [N_i, r]
                    moe_lora[b_idx, s_idx] = self.lora_B[active_adapter][f"expert{i}"](h)         # [N_i, out_features]

                moe_lora = moe_lora * self.scaling[active_adapter]

        elif self.sparse_moe_enabled:
            self._last_router_logits = None # Used to store the router logits for the loss computation
            # Compute router logits and weights
            router_logits = self.router(x_)
            router_weights = torch.softmax(router_logits, dim=-1)  # [batch, seq, num_experts]

            router_weights, selected_experts = torch.topk(router_weights, self.top_k_sparse_moe, dim=-1) # router_weights: [batch, seq, top_k_sparse_moe], selected_experts: [batch, seq, top_k_sparse_moe]
            router_weights /= router_weights.sum(dim=-1, keepdim=True) # [batch, seq, top_k_sparse_moe]

            B, S, d_in = x_.shape
            d_out = self.out_features
            K = self.top_k_sparse_moe

            weights_full = x_.new_zeros(B, S, self.num_experts)
            weights_full.scatter_(dim = 2, index = selected_experts, src = router_weights) # [batch, seq, num_experts]

            # mask_per_expert = (selected_experts[..., None] == torch.arange(self.num_experts, device=x_.device)).any(dim=-2) # [batch, seq, num_experts]
            ...
            # Compute all expert outputs
            # Collect per-expert outputs into a dense tensor: [B, S, d_out, E]
            lora_outputs = x_.new_zeros(B, S, d_out, self.num_experts)

            for i in range(self.num_experts):

                b_idx, s_idx = torch.where(mask_per_expert[...,i]) # every pair idx[k], top_x[k] is the index of the expert that is selected
                if b_idx.numel() == 0:
                    continue
                x_sel = x_[b_idx, s_idx] # [N_i, in_features]
                if self.training:
                ...
                # x_sel = self.lora_A[active_adapter][f"expert{i}"](x_sel)     # [N_i, r]
                # lora_b = self.lora_B[active_adapter][f"expert{i}"](x_sel)         # [N_i, out_features]
                # lora_outputs.append(lora_b)

                # Safety belt in case autocast toggles something:
                if out.dtype != lora_outputs.dtype:
                    out = out.to(lora_outputs.dtype)
                
                lora_outputs[b_idx, s_idx, :, i] = out
            # Weighted sum over experts
            router_weights_exp = weights_full.unsqueeze(-2)  # [batch, seq, 1, num_experts]

            moe_lora = (lora_outputs * router_weights_exp).sum(dim=-1) * self.scaling[active_adapter]

            self._last_router_logits = router_logits # Used to store the router logits for the loss computation

        # Restore shape if input was 2D
        if x.dim() == 2:
            moe_lora = moe_lora.squeeze(1)
        
        return base_out + moe_lora.to(torch_result_dtype) # Convert back to original dtype
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        ...
        # Get the standard state dict first
        state_dict = super().state_dict(destination, prefix, keep_vars)
        
        # The standard state dict should already contain all the nested ModuleDict contents
        # including router weights, but let's make sure we have everything we need
        return state_dict

    def _save_to_state_dict(self, destination, prefix, keep_vars):
        """Override to ensure proper saving of all components"""
        # Call parent method to handle standard LoRA components
        super()._save_to_state_dict(destination, prefix, keep_vars)

    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        """Custom loading that handles MoE structure"""
        # Load the rest using parent method (handles all nested ModuleDicts properly)
