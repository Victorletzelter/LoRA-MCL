...
lora_moe=True
lora_mcl_wta=False
lora_mcl_relaxed=True
lora_mcl_annealed=True
do_sampling=False
use_slurm=False
...
for i in ${ARGUMENT}; do
    ...
    if [ "${lora_mle}" = "True" ]; then

        echo "BASELINE EXPERIMENTS (Single Hypothesis)"
        WTA_MODE=wta # Default WTA mode (LoRA-MLE with 1 hyp)
        ...
        for rank in 8 $((8*i)); do
            ...
            if [ "${do_sampling}" = "True" ]; then
                ...
                for method in "${!SAMPLING_CONFIGS[@]}"; do
                    ...
                    if [ "${use_slurm}" = "True" ]; then
                    sbatch --export=ALL,WTA_MODE=${WTA_MODE},N_HYPS=1,NUM_EPOCHS=${NUM_EPOCHS},DATASET_NAME=${DATASET_NAME},CKPT_PATH=${!var_name},NUM_RETURN_SEQUENCES=${i},GENERATION_CONFIG_TOP_K=${config[0]},GENERATION_CONFIG_TOP_P=${config[1]},GENERATION_CONFIG_BEAM_SIZE=1,GENERATION_CONFIG_DO_SAMPLE=True,SEED=${SEED},GENERATION_CONFIG_TYPICAL_P=${config[2]},NAME=${method},MAX_LENGTH=${MAX_LENGTH},RANK=${rank} qwen_eval.slurm
                    else
                        echo ${!var_name}
                        launch_shell_script \
                            --n_hyps=1 \
            ...
            if [ "${tta}" = "True" ]; then
                ...
                for j in ${!TIME_MASK_PERCENTAGE_VALUES[@]}; do
                    ...
                    if [ "${use_slurm}" = "True" ]; then
                        sbatch --export=ALL,WTA_MODE=${WTA_MODE},N_HYPS=1,NUM_EPOCHS=${NUM_EPOCHS},DATASET_NAME=${DATASET_NAME},CKPT_PATH=${!var_name},NUM_RETURN_SEQUENCES=${i},GENERATION_CONFIG_TOP_K=${TOP_K_VANILLA},GENERATION_CONFIG_TOP_P=${TOP_P_VANILLA},GENERATION_CONFIG_BEAM_SIZE=${i},GENERATION_CONFIG_DO_SAMPLE=False,SEED=${SEED},GENERATION_CONFIG_TYPICAL_P=${TYPICAL_P0},NAME=greedyttaspecaugment,GAUSSIAN_NOISE_STD=0.0,TTA_ENABLED=True,TTA_MODE=spec_augment,TIME_MASK_PERCENTAGE=${value_time},FREQ_MASK_PERCENTAGE=${value_freq},GREEDY_SH_TTA=Single,MAX_LENGTH=${MAX_LENGTH},RANK=${rank} qwen_eval.slurm
                    else
                        echo ${!var_name}
                        launch_shell_script \
                            --n_hyps=1 \
                            ...
                            --greedy_sh_tta=Single # This parameter is used to bypass the errors in HF library when num_beam < num_return_sequences. It ensures that the evaluation is performed with beam size 1 in each forward pass.
                    fi
                    ...
                    if [ "${use_slurm}" = "True" ]; then
                        ...
                        launch_shell_script \
                            ...
                            --time_mask_percentage=${value_time} \
                            --freq_mask_percentage=${value_freq} \
                            --greedy_sh_tta=Double # This parameter is used to bypass the errors in HF library when num_beam < num_return_sequences. It ensures that the evaluation is performed with beam size 2 in each forward pass.
                    fi
                    
                    # Beam Search with beam size = i in each forward pass
                    for beam_size in ${i}; do
                        if [ "${use_slurm}" = "True" ]; then
                            sbatch --export=ALL,WTA_MODE=${WTA_MODE},N_HYPS=1,NUM_EPOCHS=${NUM_EPOCHS},DATASET_NAME=${DATASET_NAME},CKPT_PATH=${!var_name},NUM_RETURN_SEQUENCES=${i},GENERATION_CONFIG_TOP_K=${TOP_K_VANILLA},GENERATION_CONFIG_TOP_P=${TOP_P_VANILLA},GENERATION_CONFIG_BEAM_SIZE=${beam_size},GENERATION_CONFIG_DO_SAMPLE=False,SEED=${SEED},GENERATION_CONFIG_TYPICAL_P=${TYPICAL_P0},NAME=bsttaspecaugment,GAUSSIAN_NOISE_STD=0.0,TTA_ENABLED=True,TTA_MODE=spec_augment,TIME_MASK_PERCENTAGE=${value_time},FREQ_MASK_PERCENTAGE=${value_freq},MAX_LENGTH=${MAX_LENGTH},RANK=${rank} qwen_eval.slurm
                        else
                            echo ${!var_name}
                            launch_shell_script \
                                --n_hyps=1 \
                                ...
                                --time_mask_percentage=${value_time} \
                                --freq_mask_percentage=${value_freq} \
                                --greedy_sh_tta=False # Here, beam_size = i so this parameter is not used.
                        fi
                    done
                done
            fi
    ...
    if [ "${lora_mcl_relaxed}" = "True" ]; then
        ...
        for epsilon in "${EPSILON_VALUES[@]}"; do
            ...
            for beam_size in 1 2 ${i}; do
                if [ "${use_slurm}" = "True" ]; then
                    sbatch --export=ALL,WTA_MODE=${WTA_MODE},N_HYPS=${i},NUM_EPOCHS=${NUM_EPOCHS},DATASET_NAME=${DATASET_NAME},CKPT_PATH=${!var_name},NUM_RETURN_SEQUENCES=1,GENERATION_CONFIG_TOP_K=${TOP_K_VANILLA},GENERATION_CONFIG_TOP_P=${TOP_P_VANILLA},GENERATION_CONFIG_BEAM_SIZE=${beam_size},GENERATION_CONFIG_DO_SAMPLE=False,EPSILON=${epsilon},SEED=${SEED},GENERATION_CONFIG_TYPICAL_P=${TYPICAL_P0},NAME=greedy,MAX_LENGTH=${MAX_LENGTH} qwen_eval.slurm
                else
                    echo ${!var_name}
                    launch_shell_script \
                        --n_hyps=${i} \
    ...
    #===========================================================================
    if [ "${lora_mcl_annealed}" == "True" ]; then
        echo "ANNEALED WTA EXPERIMENTS"
        WTA_MODE=annealed-wta
        var_name="CKPT_${i}_HYP_lora_mcl_annealed_${DATASET_NAME^^}"
        echo "CKPT_PATH: ${!var_name}"
        # Beam Search with different beam sizes
        for beam_size in 1 2 ${i}; do
            if [ "${use_slurm}" = "True" ]; then
