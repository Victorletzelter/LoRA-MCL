...
def _spider_from_outputs(
    ...
    spider_outs_corpus = cider_d_outs_corpus.copy()  # Create a copy to avoid modifying the original
    spider_outs_corpus.update(spice_outs_corpus)
    spider_outs_corpus.update({"spider": spider_score})

    spider_outs_sents = cider_d_outs_sents.copy()    # Same for the sentences dictionary
    spider_outs_sents.update(spice_outs_sents)
    spider_outs_sents.update({"spider": spider_scores})
