...
def fer(
    ...
    if return_all_scores:
        ...
        if return_probs:
            ...
            fer_outs_corpus.update(probs_outs_corpus)     
            fer_outs_sents.update(probs_outs_sents)
...
def __load_pretrain_echecker(
    ...
    if os.path.isfile(echecker_model):
        file_path = echecker_model
    else:
    ...
    if 'LOCAL_CACHE' in os.environ and os.environ["LOCAL_CACHE"] not in model_type:
        model_type = os.path.join(os.environ["LOCAL_CACHE"], model_type.split("/local_cache/")[-1])
