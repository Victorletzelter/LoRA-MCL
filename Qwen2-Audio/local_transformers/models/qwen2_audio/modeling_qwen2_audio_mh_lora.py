...
class Qwen2AudioForConditionalGeneration_MH_Lora(Qwen2AudioPreTrainedModel, GenerationMixin):
    def __init__(self, config: Qwen2AudioConfig):
        super().__init__(config)

        ### Sparse MoE related params
        self.sparse_moe_enabled = config.sparse_moe_enabled
        self.router_aux_loss_coef = config.router_aux_loss_coef
        self.num_experts = config.num_experts
        self.topk = config.top_k_sparse_moe
        #########################
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if wta_training_mode == 'wta':
                ...
                idx_winner = torch.randint(0, self.num_hyps, (batch_size,))
                loss = loss[idx_winner, torch.arange(batch_size)]
                loss = loss.mean()
                ...
                loss = loss.mean()
            elif wta_training_mode == 'random-wta-relaxed':
                idx_winner = torch.randint(0, self.num_hyps, (batch_size,))
                wta_loss = torch.min(loss, dim=0).values # shape (batch_size)
                epsilon = self.wta_params_epsilon
                loss = (1 - epsilon - epsilon/(self.num_hyps - 1)) * wta_loss + (epsilon/(self.num_hyps - 1)) * loss.sum(dim = 0) # shape (batch_size)
        ...
        if self.sparse_moe_enabled and loss is not None:
            # import router_losses.py that is in models/router_losses.py
            from ..router_losses import MixtralRouterLoss
            router_logits_list = []
            for m in self.language_model.modules():
                from local_peft.tuners.lora.layer import MoELoraLinear
                if isinstance(m, MoELoraLinear) and m._last_router_logits is not None:
                    router_logits_list.append(m._last_router_logits)

            if router_logits_list:
                logits_list = []
                # mask_list = []
                for logits in router_logits_list:            # logits: [B, S, E]
                    mask = attention_mask                    # [B, S]
                    if mask is not None and mask.shape[1] != logits.shape[1]:
                        mask = mask[:, -logits.shape[1]:]    # align if needed
                    logits_list.append(logits)
                    # mask_list.append(mask)
                logits = torch.cat(logits_list, dim=0) # [B*num_layers, S, E]
                logits = logits.view(-1, logits.size(-1)) # [B*num_layers*S, E]
                aux_loss = MixtralRouterLoss(router_aux_loss_coef=self.router_aux_loss_coef, num_experts=self.num_experts, top_k=self.topk)(logits, mask)
                loss = loss + aux_loss
    ...
    def _set_adapter(self, adapter_name):
        ...
        for module in self.modules():
            if isinstance(module, LoraLayer):
                assert module.active_adapters == [adapter_name]
...
__all__ = ["Qwen2AudioForConditionalGeneration_MH_Lora", "Qwen2AudioPreTrainedModel", "Qwen2AudioEncoder"]
