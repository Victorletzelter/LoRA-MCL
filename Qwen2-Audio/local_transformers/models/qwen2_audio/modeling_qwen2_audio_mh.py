...
# Copied from local_transformers.models.whisper.modeling_whisper.WhisperAttention with Whisper->Qwen2Audio
class Qwen2AudioAttention(nn.Module):
    ...
    # Copied from local_transformers.models.bart.modeling_bart.BartAttention._shape with BART->whisper
...
# Copied from local_transformers.models.whisper.modeling_whisper.WhisperFlashAttention2 with Whisper->Qwen2Audio
...
# Copied from local_transformers.models.whisper.modeling_whisper.WhisperSdpaAttention with Whisper->Qwen2Audio
    ...
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                ...
                ' implementation, but specifying the manual implementation will be required from local_transformers version v5.0.0 onwards. This warning can be removed using the argument `attn_implementation="eager"` when loading the model.'
            )
...
# Copied from local_transformers.models.whisper.modeling_whisper.WhisperEncoderLayer with Whisper->Qwen2Audio, WHISPER->QWEN2AUDIO
...
# Copied from local_transformers.models.whisper.modeling_whisper.WhisperEncoder with Whisper->Qwen2Audio
class Qwen2AudioEncoder(Qwen2AudioPreTrainedModel):
    ...
    def __init__(self, config: Qwen2AudioEncoderConfig):
        ...
        # self.max_source_positions_pretrained = 500
        self.max_source_positions_pretrained = config.max_source_positions
        self.embed_scale = math.sqrt(embed_dim) if config.scale_embedding else 1.0

        self.conv1 = nn.Conv1d(self.num_mel_bins, embed_dim, kernel_size=3, padding=1)
        self.conv2 = nn.Conv1d(embed_dim, embed_dim, kernel_size=3, stride=2, padding=1)

        self.embed_positions = nn.Embedding(self.max_source_positions_pretrained, embed_dim)
    ...
    def _load_from_state_dict(
        ...
        # If the checkpoint's positional embeddings have a different size than expected,
        # slice them before loading to avoid allocating extra GPU memory.
        embed_pos_key = prefix + "embed_positions.weight"
        if embed_pos_key in state_dict:
            loaded_weight = state_dict[embed_pos_key]
            if loaded_weight.size(0) != self.max_source_positions:
                state_dict[embed_pos_key] = loaded_weight[:self.max_source_positions, :]
...
class Qwen2AudioMultiModalProjector_with_lora(nn.Module):
    def __init__(self, config: Qwen2AudioConfig, rank: int = 8, alpha: float = 8.0):
        super().__init__()
        # Original linear layer (can be frozen)
        self.linear = nn.Linear(config.audio_config.d_model, config.text_config.hidden_size, bias=True)
        ...
        self.lora_A = nn.Linear(config.audio_config.d_model, self.lora_rank, bias=False)
        self.lora_B = nn.Linear(self.lora_rank, config.text_config.hidden_size, bias=False)
    ...
    def forward(self, audio_features):
        ...
        base_output = self.linear(audio_features)
        
        # LoRA transformation
        lora_output = self.lora_B(self.lora_A(audio_features)) * self.lora_scaling
        
        # Combine outputs
        hidden_states = base_output + lora_output
        return hidden_states

    def enable_lora_training(self):
        """Freeze the original weights and enable training for LoRA weights"""
    ...
    def disable_lora_training(self):
        """Enable training for original weights and disable for LoRA weights"""
        # Unfreeze original linear layer
        for param in self.linear.parameters():
            param.requires_grad = True
            
        # Disable training for LoRA layers
        for param in self.lora_A.parameters():
            param.requires_grad = False
        for param in self.lora_B.parameters():
            param.requires_grad = False
class Qwen2AudioMultiModalProjector_with_drop(nn.Module):
    def __init__(self, dropout: float, config: Qwen2AudioConfig):
        super().__init__()
        self.linear = nn.Linear(config.audio_config.d_model, config.text_config.hidden_size, bias=True)
...
class Qwen2AudioForConditionalGeneration_MH(Qwen2AudioPreTrainedModel, GenerationMixin):
    def __init__(self, config: Qwen2AudioConfig):
        super().__init__(config)
        ...
        self.try_new_version_adapter_setting = config.try_new_version_adapter_setting
        self.audio_tower = AutoModel.from_config(config.audio_config)
        self.num_hyps = config.num_hyps
        self.orig_layer_names_one_hyp = config.orig_layer_names_one_hyp
        if config.lora_enabled is True:
            if self.num_hyps == 1 and self.orig_layer_names_one_hyp is True:
                self.multi_modal_projector = Qwen2AudioMultiModalProjector_with_lora(config, rank=config.lora_rank, alpha=config.lora_alpha)
            else:
                self.multi_modal_projectors = nn.ModuleList([Qwen2AudioMultiModalProjector_with_lora(config, rank=config.lora_rank, alpha=config.lora_alpha) for _ in range(self.num_hyps)])
        elif config.dropout_enabled is True:
            if self.num_hyps == 1 and self.orig_layer_names_one_hyp is True:
                self.multi_modal_projector = Qwen2AudioMultiModalProjector_with_drop(config.dropout, config)
            else:
                self.multi_modal_projectors = nn.ModuleList([Qwen2AudioMultiModalProjector_with_drop(config.dropout, config) for _ in range(self.num_hyps)])
        else : 
            if self.num_hyps == 1 and self.orig_layer_names_one_hyp is True:
                self.multi_modal_projector = Qwen2AudioMultiModalProjector(config)
            else:
                self.multi_modal_projectors = nn.ModuleList([Qwen2AudioMultiModalProjector(config) for _ in range(self.num_hyps)])
    ...
    # Copied from local_transformers.models.llava.modeling_llava.LlavaForConditionalGeneration.get_input_embeddings
    def get_input_embeddings(self):
        return self.language_model.get_input_embeddings()

    # Copied from local_transformers.models.llava.modeling_llava.LlavaForConditionalGeneration.set_input_embeddings
    def set_input_embeddings(self, value):
        self.language_model.set_input_embeddings(value)

    # Copied from local_transformers.models.llava.modeling_llava.LlavaForConditionalGeneration.get_output_embeddings
    def get_output_embeddings(self):
        return self.language_model.get_output_embeddings()

    # Copied from local_transformers.models.llava.modeling_llava.LlavaForConditionalGeneration.set_output_embeddings
    def set_output_embeddings(self, new_embeddings):
        self.language_model.set_output_embeddings(new_embeddings)

    # Copied from local_transformers.models.llava.modeling_llava.LlavaForConditionalGeneration.set_decoder
    def set_decoder(self, decoder):
        self.language_model.set_decoder(decoder)

    # Copied from local_transformers.models.llava.modeling_llava.LlavaForConditionalGeneration.get_decoder
    ...
    def forward(
        ...
        >>> import librosa
        >>> from local_transformers import AutoProcessor, Qwen2AudioForConditionalGeneration_MH

        >>> model = Qwen2AudioForConditionalGeneration_MH.from_pretrained("Qwen/Qwen2-Audio-7B")
        ...
        if self.num_hyps == 1 and self.orig_layer_names_one_hyp is True:
            ...
            if hypothesis_idx is None:
                list_hyps = range(len(self.multi_modal_projectors))
            else:
        ...
        if inputs_embeds is None:
            ...
            ####### TODO: hypothesis_idx management
            inputs_embeds_dict = {}

            if input_features is None :
                assert len(list_hyps) == 1, "If input_features is None, then there should only be one hypothesis: We are in generation mode."
                inputs_embeds_dict[list_hyps[0]] = inputs_embeds

            # 2. Merge text and audios
            if input_features is not None and input_ids.shape[1] != 1:
                # if input_features.ndim == 4:
                    # assert input_features.shape[1] == 1, "input_features should have 1 channel"
                ...
                if audio_feat_lengths.ndim == 1:
                    lengths_expand = audio_feat_lengths.unsqueeze(1).expand(batch_size, max_seq_len)
                    # Create mask
                else:
                    lengths_expand = audio_feat_lengths
                ...
                for hypothesis_idx in list_hyps:
                    if self.orig_layer_names_one_hyp is True and self.num_hyps == 1:
                        audio_features = self.multi_modal_projector(selected_audio_feature)
                    else:
                        audio_features = self.multi_modal_projectors[hypothesis_idx](selected_audio_feature)
                    ...
                    if legacy_processing:
                        ...
                        inputs_embeds_dict[hypothesis_idx], attention_mask, labels, position_ids, _ = self._merge_input_ids_with_audio_features(
                        ...
                        inputs_embeds_dict[hypothesis_idx] = inputs_embeds.masked_scatter(special_audio_mask, audio_features)
        ...
        for hypothesis_idx in list_hyps:
            outputs = self.language_model(
                ...
                inputs_embeds=inputs_embeds_dict[hypothesis_idx],
                use_cache=use_cache,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
                return_dict=return_dict)
            logits_dict[hypothesis_idx] = outputs[0]
        ...
        if labels is not None:
            ...
            for elt in range(batch_size):
                if attention_mask is not None:
                    shift_attention_mask.append(attention_mask[elt, 1:])
                    shift_labels.append(labels[elt, 1:][shift_attention_mask[-1].to(labels.device) != 0].contiguous())
                else:
                    shift_labels.append(labels[elt, 1:].contiguous())
                mask.append(shift_labels[-1] != -100)
            ...
            for hypothesis_idx in list_hyps:
                ...
                for elt in range(batch_size):
                    if attention_mask is not None:
                        shift_logits = logits_dict[hypothesis_idx][elt, :-1][shift_attention_mask[elt].to(logits_dict[hypothesis_idx].device) != 0].contiguous()                
                    else:
                        shift_logits = logits_dict[hypothesis_idx][elt, :-1].contiguous()
                    mask[elt] = mask[elt].view(-1).to(shift_logits.device)
                    loss = loss_fct(
                        shift_logits.view(-1, shift_logits.size(-1))[mask[elt],:], shift_labels[elt].view(-1)[mask[elt]].to(shift_logits.device)
                    )
                    loss_list_batch.append(loss.mean()) # We average over the sequence length
                loss_list.append(torch.stack(loss_list_batch, dim=0)) # shape (batch_size) for each hypothesis
            
            loss = torch.stack(loss_list, dim=0) # shape (num_hypotheses, batch_size)

            if wta_training_mode == 'wta':
                loss = torch.min(loss, dim=0).values.mean()
            elif wta_training_mode == 'relaxed-wta':
                wta_loss = torch.min(loss, dim=0).values # shape (batch_size)
                epsilon = self.wta_params_epsilon
                loss = (1 - epsilon - epsilon/(self.num_hyps - 1)) * wta_loss + (epsilon/(self.num_hyps - 1)) * loss.mean(dim = 0) # shape (batch_size)
                loss = loss.mean()
                ...
                if temperature <= self.wta_params_fin_temp:
                    ...
                    weights = torch.exp(-loss / temperature)
                    weights = weights / weights.sum(dim = 0, keepdim = True) # shape (batch_size)
        ...
        if return_dict:
            ...
            # logits_dict = torch.cat([logits_dict[hypothesis_idx] for hypothesis_idx in list_hyps], dim=0)
            output = (logits_dict[list_hyps[-1]],) + outputs[1:]
            # output = (logits_dict,) + outputs[1:]
            if loss is not None:
                all_losses = torch.stack(loss_list, dim=0)
                return (loss,) + output + (all_losses,)
            else: 
                return Qwen2AudioCausalLMOutputWithPast(loss=None,
                logits=output[0],
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        hypothesis_idx = kwargs.get("hypothesis_idx", None)
        model_inputs.update(
            {
                "position_ids": position_ids,
                "past_key_values": past_key_values,
...
__all__ = ["Qwen2AudioForConditionalGeneration_MH", "Qwen2AudioPreTrainedModel", "Qwen2AudioEncoder"]
