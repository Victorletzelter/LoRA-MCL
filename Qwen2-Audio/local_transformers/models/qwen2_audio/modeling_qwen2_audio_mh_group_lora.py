...
class Qwen2AudioForConditionalGeneration_MH_Group_Lora(Qwen2AudioPreTrainedModel, GenerationMixin):
    def __init__(self, config: Qwen2AudioConfig):
        super().__init__(config)
    ...
    def forward(
        ...
        if len(list_hyps) == 1 and self.native_group_lora_enabled:
            adapter_name = f"lora{list_hyps[0]}"
            # Load the adapter
        ...
        if self.native_group_lora_enabled and len(list_hyps) > 1:
            feature_attention_mask = feature_attention_mask.unsqueeze(1).expand(-1, self.num_hyps, -1)
            feature_attention_mask = feature_attention_mask.reshape(-1, *feature_attention_mask.shape[2:])
            original_batch_size = input_ids.shape[0]
            input_ids = input_ids.unsqueeze(1).expand(-1, self.num_hyps, -1)
        ...
        if input_features is not None and input_ids.shape[1] != 1:
            ...
            if self.native_group_lora_enabled and len(list_hyps) > 1: 
                # Duplicate audio features for all hypotheses
                input_features = input_features.unsqueeze(1).expand(-1, self.num_hyps, -1, -1)
                input_features = input_features.reshape(-1, *input_features.shape[2:]) # shape [batch*adapter_count, in_features,seq]
        ...
        if self.native_group_lora_enabled and len(list_hyps) > 1:
            ...
            for hypothesis_idx in list_hyps:
                logits_dict[hypothesis_idx] = outputs[0].reshape(original_batch_size, self.num_hyps, outputs[0].shape[-2], outputs[0].shape[-1])[:,hypothesis_idx,:,:]
            ...
            logits_dict[list_hyps[0]] = outputs[0]
        ...
        if labels is not None:
            ...
            if wta_training_mode == 'wta':
                ...
                loss = loss.mean()
            elif wta_training_mode == wta_training_mode == 'random-wta-relaxed':
                idx_winner = torch.randint(0, self.num_hyps, (batch_size,))
                wta_loss = loss[idx_winner, torch.arange(batch_size)] # shape (batch_size)
                epsilon = self.wta_params_epsilon
                loss = (1 - epsilon - epsilon/(self.num_hyps - 1)) * wta_loss + (epsilon/(self.num_hyps - 1)) * loss.sum(dim = 0) # shape (batch_size)
                loss = loss.mean()
        ...
        if self.sparse_moe_enabled and loss is not None:
            ...
            if router_logits_list:
                ...
                for logits in router_logits_list:            # logits: [B, S, E]
                    ...
                    aux_loss = aux_loss + MixtralRouterLoss(self.config)(logits, mask)
                loss = loss + aux_loss
...
__all__ = ["Qwen2AudioForConditionalGeneration_MH_Group_Lora", "Qwen2AudioPreTrainedModel", "Qwen2AudioEncoder"]
