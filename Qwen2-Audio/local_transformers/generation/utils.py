...
class GenerationMixin:
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # if "hypothesis_idx" in kwargs:
            # print('OK?')
            # print("hypothesis_idx", kwargs.get("hypothesis_idx"))
            # model_inputs["hypothesis_idx"] = kwargs["hypothesis_idx"]
    ...
    def compute_transition_scores(
        ...
        ```python
        >>> from local_transformers import GPT2Tokenizer, AutoModelForCausalLM
        >>> import numpy as np
    ...
    def generate(
        ...
        negative_prompt_ids: Optional[torch.Tensor] = None,
        negative_prompt_attention_mask: Optional[torch.Tensor] = None,
        # hypothesis_idx: Optional[torch.Tensor] = None,
        **kwargs,
        ...
        # if hypothesis_idx is not None and "hypothesis_idx" not in model_kwargs:
            # model_kwargs["hypothesis_idx"] = hypothesis_idx
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            # model_inputs.update({"hypothesis_idx": hypothesis_idx} if hypothesis_idx is not None else {})

            # if "hypothesis_idx" in model_kwargs and "hypothesis_idx" not in model_inputs:
                # model_inputs["hypothesis_idx"] = model_kwargs["hypothesis_idx"]
            ...
            if isinstance(outputs, tuple):
                next_token_logits = outputs[0][:, -1, :].clone().float()
            else:
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if isinstance(outputs, tuple): ### TODO: Make cleaner
                next_token_logits = outputs[0][:, -1, :].clone().float()
            else:
                next_token_logits = outputs.logits[:, -1, :].clone().float()
            next_token_logits = next_token_logits.to(input_ids.device)
    ...
    def _group_beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # if "hypothesis_idx" in model_inputs:
                # model_inputs.update({"hypothesis_idx": model_inputs["hypothesis_idx"]})
