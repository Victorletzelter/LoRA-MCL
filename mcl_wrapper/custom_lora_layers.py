...
class GroupLinear(nn.Module, LoraLayer):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if len(self.active_adapters) > 1: # during training
            ...
            lora_output = (x @ A_slice.T @ B_slice.T) * self.scaling['lora0'] # We assume the scaling is the same for all adapters
...
def register_mcl_custom_layers(lora_config, num_hyps: int):
    ...
    Usage:
        from mcl_wrapper.custom_lora_layers import register_mcl_custom_layers
