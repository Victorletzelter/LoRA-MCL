...
class MCLModelOutput(OrderedDict):
    ...
    def __init__(self, base_output, all_losses=None):
        ...
        if hasattr(base_output, 'items'):
            ...
            for attr in ['logits', 'loss', 'hidden_states', 'attentions', 'past_key_values', 'attention_mask']:
...
class MCLLoraConfig(PeftConfig):
    ...
    modules_to_save: Optional[List[str]] = field(
        default=None,
        metadata={"help": "List of modules apart from adapter layers to be set as trainable and saved in the final checkpoint"}
    )
    init_lora_weights: Union[bool, str] = field(
        default=True,
        metadata={"help": "How to initialize the weights of the adapter layers"}
    )
    ...
    layer_replication: Optional[List[tuple]] = field(
        default=None,
        metadata={"help": "Layer replication configuration"}
    )
    runtime_config: LoraRuntimeConfig = field(
        default_factory=LoraRuntimeConfig,
        metadata={"help": "Runtime configurations"}
    )
    lora_bias: bool = field(
        default=False,
        metadata={"help": "Whether to enable the bias term for the LoRA B parameter"}
    )
    _custom_modules: Optional[dict] = field(default=None, metadata={"help": "Custom modules for LoRA"})
    
    # MCL-specific parameters
    num_hyps: int = field(default=1, metadata={"help": "Number of hypotheses for MCL"})
    use_group_lora: bool = field(default=False, metadata={"help": "Whether to use GroupLinear for efficient multi-hypothesis training"})
    
    # WTA (Winner-Take-All) parameters
    wta_training_mode: str = field(default="wta", metadata={"help": "WTA training mode: 'wta', 'relaxed-wta', 'annealed-wta', etc."})
    wta_params_epsilon: float = field(default=0.0, metadata={"help": "Epsilon parameter for relaxed WTA"})
    wta_params_ini_temp: float = field(default=1.0, metadata={"help": "Initial temperature for annealed WTA"})
    wta_params_fin_temp: float = field(default=0.01, metadata={"help": "Final temperature for annealed WTA"})
    wta_params_decay_rate: float = field(default=0.999, metadata={"help": "Temperature decay rate"})
    wta_params_schedule_mode: str = field(default="global_step", metadata={"help": "Temperature schedule mode"})
...
class MCLForwardMixin(ABC):
    ...
    def _filter_model_kwargs(self, kwargs):
        ...
        valid_keys = {
            ...
            'use_cache', 'encoder_hidden_states', 'encoder_attention_mask',
    ...
    def model_temperature(self, global_step: Optional[int] = None, epoch_number: Optional[int] = None):
        """Compute temperature for annealed WTA"""
        if self.wta_params_schedule_mode == "global_step" and global_step is not None:
            temperature = self.wta_params_fin_temp + (self.wta_params_ini_temp - self.wta_params_fin_temp) * (self.wta_params_decay_rate ** global_step)
        elif self.wta_params_schedule_mode == "epoch" and epoch_number is not None:
            temperature = self.wta_params_fin_temp + (self.wta_params_ini_temp - self.wta_params_fin_temp) * (self.wta_params_decay_rate ** epoch_number)
        else:
            temperature = self.temperature
        return temperature
    
    def compute_wta_loss(self, loss_tensor: torch.Tensor, wta_training_mode: str, 
                        global_step: Optional[int] = None, epoch_number: Optional[int] = None) -> torch.Tensor:
        """
        ...
        if wta_training_mode == 'wta':
            ...
            loss = loss_tensor[idx_winner, torch.arange(batch_size, device=loss_tensor.device)].mean()
            ...
            loss = (1 - epsilon - epsilon/(self.num_hyps - 1)) * wta_loss + (epsilon/(self.num_hyps - 1)) * loss_tensor.sum(dim=0)
            loss = loss.mean()
    ...
    def forward(self, 
        ...
        if not self.native_group_lora_enabled:
            ...
            return self._multi_hypothesis_forward_standard(
                wta_mode=wta_mode, global_step=global_step, epoch_number=epoch_number, hypothesis_idx=hypothesis_idx,
                return_all_hyps=return_all_hyps, return_all_losses=return_all_losses,
                **kwargs
            )
        else:
    ...
    def _multi_hypothesis_forward_standard(self, wta_mode, global_step, epoch_number, hypothesis_idx,
                                         return_all_losses, **kwargs):
        ...
        for hyp_idx in list_hyps:
            ...
            if labels is not None: 
                ...
                final_loss = self.compute_wta_loss(loss_tensor=loss_tensor, wta_training_mode=wta_mode, global_step=global_step, epoch_number=epoch_number)
    ...
    def _multi_hypothesis_forward_group_lora(self, wta_mode, global_step, epoch_number, hypothesis_idx,
                                           return_all_losses, **kwargs):
        ...
        for key, value in filtered_kwargs.items():
            ...
            elif isinstance(value, torch.Tensor) and value.dim() > 0 and key in ['input_ids', 'attention_mask', 'feature_attention_mask']:
