#!/bin/bash
# Shell script to evaluate LoRA-MLE on newstest2014 test set

OUTPUT_DIR=${1:-"./alma-7b-parallel-ft-lora"}
pairs=${2:-"de-en,cs-en,is-en,zh-en,ru-en,en-de,en-cs,en-is,en-zh,en-ru"}
LORA_RANK=${3:-"16"}
NUM_SAMPLES=${4:-"16"}
NUM_RETURN_SEQUENCES=${5:-"2"}
PEFT_MODEL_ID=${OUTPUT_DIR}/best/adapter_model
...
accelerate launch --main_process_port ${port} --num_processes ${NGPUS} --config_file $PROJECT_DIR/configs/deepspeed_eval_config_zero3_bf16.yaml \
     $PROJECT_DIR/run_llmmt.py \
    --model_name_or_path haoranxu/ALMA-7B-Pretrain \
    --text_test_file $PROJECT_DIR/outputs/newstest2014/wmt-testset/ende/wmt14-en-de.extra_refs.tok \
