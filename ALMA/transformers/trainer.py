...
class Trainer:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss = self.compute_loss(model, inputs, global_step=self.state.global_step, epoch_number=self.state.epoch)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, global_step=None, epoch_number=None):
        """
        ...
        if global_step is not None and epoch_number is not None:
            ...
            inputs["epoch_number"] = epoch_number
            inputs["wta_training_mode"] = model.wta_training_mode if hasattr(model, "wta_training_mode") else None
        else:
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            if step % 10 == 0:
                logger.info(f"Step {step} of {len(dataloader)}")
            # if step > 100:
                # logger.info(f"Step {step} of {len(dataloader)}. Stopping early.")
                # break
            ...
            if logits is not None:
                if logits.dim() == 3:
                    logits = self.accelerator.pad_across_processes(logits, dim=2, pad_index=-100)
