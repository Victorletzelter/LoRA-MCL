#!/usr/bin/env python
# This file is adapted from the ALMA repository https://github.com/fe1ixxu/ALMA/blob/master/utils/utils.py
# Under MIT License.
...
from transformers.models.llama.modeling_llama_mh_lora import LlamaForCausalLM_MH_Lora
from transformers.models.llama.modeling_llama_mh_group_lora import LlamaForCausalLM_MH_Group_Lora
...
def load_wmt_multi_ref_file(pairs, data_args, model_args):
    """
    Load a WMT-style multi-reference file with lines like:
      S-0<TAB>source
      T-0<TAB>main reference
      R1-0<TAB>extra reference 1
      ...
    Keeps multiple references for metrics in `mrefs` per example,
    while `translation[tgt_lang]` holds the primary target (T-*).
    """
    assert len(pairs) == 1, "Multi-ref file only supports one translation direction!"
    src_lang, tgt_lang = list(pairs)[0].split("-")

    path = data_args.text_test_file
    sources = {}
    primary_t = {}
    extra_refs = {}
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if not line:
                continue
            # Expect PREFIX-ID<TAB>TEXT
            try:
                key, text = line.split("\t", 1)
            except ValueError:
                continue
            # key examples: "S-0", "T-0", "R3-0"
            try:
                prefix, idx_str = key.split("-", 1)
                idx = int(idx_str)
            except Exception:
                continue
            ...
            if prefix == "S":
                ...
                extra_refs.setdefault(idx, []).append(text)

    # Build HF dataset rows in order
    max_idx = max(sources.keys() | primary_t.keys() | extra_refs.keys()) if (sources or primary_t or extra_refs) else -1
    data = []
    for i in range(max_idx + 1):
        src_text = sources.get(i, "")
        t_text = primary_t.get(i, "")
        refs = extra_refs.get(i, [])

        # mrefs = [primary T] + extras (dedup, keep order)
        mrefs = []
        # if t_text:
            # mrefs.append(t_text)
        for r in refs:
            # if r and r not in mrefs:
            mrefs.append(r)

        # Fallback: if no primary T, still keep refs (or empty list)
        if not mrefs:
            mrefs = []

        row_dict = {}

        row_dict[src_lang+'-'+tgt_lang] = {
            src_lang: src_text,
            tgt_lang: t_text,
            # "translation": {src_lang: src_text, tgt_lang: t_text},
            "mrefs": mrefs,  # list of strings (multiple references)
        }
        data.append(row_dict)

    from datasets import Dataset
    test_ds = Dataset.from_list(data)

    pair = f"{src_lang}-{tgt_lang}"
    return {pair: {"test": test_ds}}
...
def load_model(data_args, model_args, training_args, tokenizer, logger):
    ...
    config.num_hyps = model_args.num_hyps
    if model_args.is_mcl:
        config.wta_training_mode = model_args.wta_training_mode
        config.wta_params_epsilon = model_args.wta_params_epsilon
        config.wta_params_ini_temp = model_args.wta_params_ini_temp
        config.wta_params_fin_temp = model_args.wta_params_fin_temp
        config.wta_params_decay_rate = model_args.wta_params_decay_rate
        config.wta_params_schedule_mode = model_args.wta_params_schedule_mode
    ...
    if model_args.model_name_or_path:
        ...
        if model_args.is_mcl:
            logger.info('MCL enabled')
            if model_args.is_group_lora:
                logger.info(f"Loading MCL Group Lora model from {model_args.model_name_or_path}")
                AutoModelLoad = LlamaForCausalLM_MH_Group_Lora
            else:
                logger.info(f"Loading MCL Lora model from {model_args.model_name_or_path}")
                AutoModelLoad = LlamaForCausalLM_MH_Lora
        else:
    ...
    if model_args.use_peft:
        if model_args.is_mcl:
            ...
            # else:
            # rank = model_args.lora_rank/model_args.num_hyps if model_args.lora_rank % model_args.num_hyps == 0 else model_args.lora_rank
            # rank = int(rank)
            ...
            config.num_hyps = model_args.num_hyps
            config.use_group_lora = model_args.is_group_lora
            model = get_peft_model(model, config, adapter_name="lora0")
            for i in range(1, model_args.num_hyps):
                model.add_adapter(f"lora{i}", config)
            
            if model_args.peft_model_id:
                logger.info(f"Loading adapters from {model_args.peft_model_id}")
                for i in range(model_args.num_hyps):
                    model.load_adapter(os.path.join(model_args.peft_model_id, f"lora{i}"), adapter_name=f"lora{i}", is_trainable=training_args.do_train, torch_device=f"cuda:{int(os.getenv('LOCAL_RANK', 0))}")
