...
class MCLModelWrapper:
    ...
    def __init__(
        self,
        mcl_lora_config: Optional[MCLLoraConfig] = None,
    ...
    def wrap_model(
        ...
        **model_kwargs,
        ...
        if (
            "max_source_positions" in model_kwargs
        ):  # Specfic case of Qwen2-Audio model, where the max source positions are provided as a model_kwargs.
        ...
        if (
            len(loading_kwargs) == 0
        ):  # If no loading kwargs are provided, use low cpu mem usage and bfloat16 dtype by default
            loading_kwargs = {
        ...
        model = mcl_model_class.from_pretrained(
            pretrained_repo, config=config, **loading_kwargs, **model_kwargs
        )
        ...
        if (
            pretrained_repo != model_name_or_path
        ):  # In this case, we can to load LoRA adapters from the model_name_or_path
            ...
            num_hyps_to_load = (
                model_kwargs["num_hyps"]
                if "native_group_lora_enabled" not in model_kwargs
                or model_kwargs["native_group_lora_enabled"] is False
                else 1
            )
            ...
            for k in range(num_hyps_to_load):
                model.load_adapter(
                    os.path.join(model_name_or_path, f"lora{k}"),
                    adapter_name=f"lora{k}",
                    torch_device="cuda:0",
                )
    ...
    def _apply_lora(self, model):
        ...
        num_adapter_to_add = (
            self.mcl_lora_config.num_hyps if self.mcl_lora_config.use_group_lora is False else 1
        )
...
def get_peft_mcl(
    ...
    **model_kwargs,
    ...
    wrapper = MCLModelWrapper(mcl_lora_config=mcl_lora_config, mcl_model_config=mcl_model_config)
