...
def eval_model(args):
    '''test_tokenizer = AACTokenizer(
        ...
        normalize=True,
        backend="ptb",
        #cache_path=os.environ["AAC_METRICS_CACHE"],
        #java_path=os.environ["JAVA_PATH"],
        #tmp_path="/tmp"
    )
    evaluator = MH_AACEvaluator(subrun_path=None,test_tokenizer=test_tokenizer)   
    # Model'''
    disable_torch_init()
    ...
    if hasattr(args, 'lora_weights') and args.lora_weights:
        ...
        lora_config.use_group_lora = True #False #True
    ...
    for thresh in threshs:
        ...
        for j,line in enumerate(tqdm(questions)):
            ...
            with torch.inference_mode():
                ...
                output_ids = model.generate(
                    ...
                    use_cache=True)
                    #outputs.append(tokenizer.batch_decode(output_ids, skip_special_tokens=True)[0].strip())
                outputsD=[tokenizer.batch_decode(output_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[i].strip() for i in range(output_ids.shape[0])]#.append(tokenizer.batch_decode(output_ids, skip_special_tokens=True)[0].strip())
        ...
        import pickle 
        path = f'./checkpoints/'+args.lora_weights.split('/')[-1]+'/'+args.lora_weights.split('/')[-1]+'_'+str(thresh)+'RepPenBoosted'+str(args.num_boost)+'SINGLEHYP.pkl'
