...
def eval_model(args):
    ...
    def set_expert_specific_flag(model,exp):
            for name, module in model.named_modules():
                ...
                if hasattr(module, 'lora_A') or hasattr(module, 'lora_B'):
                    ...
                    if hasattr(module, 'expert_specific'):
                        module.expert_specific = exp
                    else:
                        setattr(module, 'expert_specific', 1)
                        print(f"Set expert_specific=1 for: {name}")
    ...
    for thresh in threshs:
        ...
        for j,line in enumerate(tqdm(questions)):
            ...
            with torch.inference_mode():
                ...
                for exp in range(args.num_beams):
                    set_expert_specific_flag(model,exp)
        ...
        import pickle 
        path = f'./checkpoints/'+args.lora_weights.split('/')[-1]+'/'+args.lora_weights.split('/')[-1]+'_'+str(thresh)+'FIXEDEXPRepPenBoosted'+str(args.num_boost)+'.pkl'
