...
class Qwen2AudioForConditionalGeneration_MH(Qwen2AudioPreTrainedModel, GenerationMixin):
    def __init__(self, config: Qwen2AudioConfig):
        super().__init__(config)
        self.corrected_batch_wise_loss = config.corrected_batch_wise_loss
    ...
    def forward(
        ...
        >>> import librosa
        >>> from transformers import AutoProcessor, Qwen2AudioForConditionalGeneration_MH
        ...
        if labels is not None:
            if self.corrected_batch_wise_loss :
                shift_labels = []
                mask = []
            ...
            for hypothesis_idx in list_hyps:
                ...
                if self.corrected_batch_wise_loss :
                    ...
                    if attention_mask is not None:
                        # shift_attention_mask = attention_mask[..., 1:]
                        shift_logits = logits_dict[hypothesis_idx][..., :-1, :][shift_attention_mask.to(logits_dict[hypothesis_idx].device) != 0].contiguous()
                        # shift_labels = labels[..., 1:][shift_attention_mask.to(labels.device) != 0].contiguous()
                    else:
                        shift_logits = logits_dict[hypothesis_idx][..., :-1, :].contiguous()
                        # shift_labels = labels[..., 1:].contiguous()
                    mask = mask.view(-1).to(shift_logits.device)
                    # Flatten the tokens
                    loss = loss_fct(
                        shift_logits.view(-1, shift_logits.size(-1))[mask,:], shift_labels.view(-1)[mask].to(shift_logits.device)
                    )
                    loss_list.append(loss.mean()) # We average over the sequence length
