...
class Trainer:
    ...
    def training_step(
        ...
        if hasattr(self.model,"wta_training_mode") and self.model.wta_training_mode == "annealed-wta":
            temperature = self.model.model_temperature(global_step=self.state.global_step, epoch_number=self.state.epoch)
            my_temperature_dict = {"temperature": temperature}
            self.log(my_temperature_dict)
        ...
        with self.compute_loss_context_manager():
            ...
            # else:
            loss = self.compute_loss(model, inputs, num_items_in_batch=num_items_in_batch, global_step=self.state.global_step, epoch_number=self.state.epoch)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None, global_step=None, epoch_number=None, eval_mode=False):
        """
        ...
        if self.model_accepts_loss_kwargs:
            ...
            if global_step is not None:
                loss_kwargs["global_step"] = global_step
            if epoch_number is not None:
                loss_kwargs["epoch_number"] = epoch_number
            inputs = {**inputs, **loss_kwargs}
        if global_step is not None and epoch_number is not None:
            loss_new_kwargs = {}
            loss_new_kwargs["global_step"] = global_step
            loss_new_kwargs["epoch_number"] = epoch_number
            inputs = {**inputs, **loss_new_kwargs}
        else :
            if model.wta_training_mode != "wta":
    ...
    def evaluation_loop(
        ...
        all_losses_list = EvalLoopContainer(self.args.eval_do_concat_batches, padding_index=-100)
        # all_losses_list = []
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            losses, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys, eval_mode=True)
            main_input_name = getattr(self.model, "main_input_name", "input_ids")
            inputs_decode = (
            ...
            if logits is not None:
                ### TODO: Fix dynamic cache issue
                if len(logits) >= 3:
                    all_losses_list.add(logits[-1])
                    # all_losses_list.append(logits[-1].cpu())
                if len(logits) > 1:
                    logits = logits[0]
            ...
            if self.args.batch_eval_metrics:
                if self.compute_metrics is not None and logits is not None and labels is not None:
                    ...
                    metrics["temperature"] = self.model.model_temperature(global_step=self.state.global_step, epoch_number=self.state.epoch)
            ...
            # Gather all tensors and put them back on the CPU if we have done enough accumulation steps.
                ...
                if all_losses_list is not None:
                    all_losses_list.to_cpu_and_numpy()
        ...
        if all_losses_list is not None:
            all_losses_list = all_losses_list.get_arrays()
        ...
        if (
            ...
            eval_set_kwargs["all_losses"] = all_losses_list if "all_losses" in args.include_for_metrics else None
            eval_set_kwargs["n_samples"] = num_samples
            if all_losses_list is not None:
                eval_set_kwargs["n_hyps"] = all_losses_list.shape[0] // num_samples
            else:
                eval_set_kwargs["n_hyps"] = 1
            metrics = self.compute_metrics(
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        loss, outputs = self.compute_loss(model, inputs, return_outputs=True, eval_mode=eval_mode)
                    ...
                    with self.compute_loss_context_manager():
                        if eval_mode:
                            inputs["wta_training_mode"] = "wta"
                        if model.try_new_version_adapter_setting:
                            all_losses = []
                            for hypothesis_idx in range(model.num_hyps):
                                model.set_adapter(f"lora{hypothesis_idx}")
                                outputs = model(**inputs)
                                all_losses.append(outputs[-1]) # shape (batch_size)
                            all_losses = torch.stack(all_losses, dim=0) # shape (num_hyps, batch_size)
                            outputs = (all_losses.min(dim=0).values.mean(),) + outputs[1:-1] + (all_losses,)
                        else:
                            outputs = model(**inputs)
                    if isinstance(outputs, dict):
